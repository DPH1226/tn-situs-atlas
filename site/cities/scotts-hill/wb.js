/* Situs Workbench — city view. Data: window.WB from wbdata.js. */
(function () {
  "use strict";
  ...
  function loadLocal() { try { var s = localStorage.getItem("civvix.adj.wilson"); if (s) ADJ = JSON.parse(s) || {}; } catch (e) {} }
  function saveLocal() { try { localStorage.setItem("civvix.adj.wilson", JSON.stringify(ADJ)); } catch (e) {} }
  loadLocal();
  ...
  function renderEvidence(b) {
    ...
    host.innerHTML =
      ...
      "<dt>Postal city</dt><dd>" + esc(q[F.post_city] || "—") + (q[F.postal_county] && (W.homes || ["WILSON"]).indexOf(q[F.postal_county]) < 0 ? ' <span class="bad">→ ' + esc(q[F.postal_county]) + " COUNTY</span>" : "") + "</dd>" +
  ...
  function renderDor() {
    ...
    host.innerHTML =
      ...
      '<div id="dor-out"></div>' +
      '<div class="sec"><h3>What the request asks for</h3><ul><li>Situs code' + ((W.own_codes||[]).length===1?'':'s') + ' for ' + esc(W.county_label||W.county) + ': <span class="mono">' + (W.own_codes||[]).join(' ') + '</span></li><li>Name · DBA · account id · physical address · mailing address · ZIP · situs · status · registration date · location id · as-of date</li><li>Situs-code table, correction procedure, lookback authority, REP access, contractor requirements</li></ul><p>Cities request their own situs report from the Department under T.C.A. § 67-1-1704(d); the ZIP list is the city\'s ZIPs plus the surrounding ring.</p></div>';
  ...
  function joinDor() {
    ...
    $("dor-out").innerHTML = '<div class="stat"><div><div class="v">' + nf(dorRows.length) + '</div><div class="k">DOR rows</div></div><div><div class="v">' + nf(matched) + '</div><div class="k">matched to a rooftop</div></div><div><div class="v" style="color:var(--sev-crit)">' + nf(mism) + '</div><div class="k">coded ≠ measured</div></div></div>' +
      ...
      (unmatched.length ? "<p><b>" + nf(unmatched.length) + " rows did not match</b> a rooftop-placed business by name. These are either outside the public business layer, registered under a different name, or not physically in " + esc(W.county_label || W.county) + " — every one is worth a look, because a business coded to " + esc(W.county_label || W.county) + " with no rooftop there is the reverse error.</p><div class=\"zips\" style=\"max-height:180px;overflow:auto\">" + unmatched.slice(0, 200).map(function (u) { return esc(u[0]) + " · " + esc(u[1]) + " · " + esc(u[2]); }).join("<br>") + "</div>" : "");
  ...
  if (W.county) { var cc = document.querySelector(".top .county"); if (cc) cc.textContent = W.chip || (W.county + " · TN · " + SITCODES[0] + "–" + SITCODES[SITCODES.length - 1]); }
  /* ================= boot ================= */
