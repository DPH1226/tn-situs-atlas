...
if _fp.exists():
    for _c in json.loads(_fp.read_text()).get("counties", []):
        FLOWS[_c["jurisdiction"].upper().replace(" ", "")] = {"owed": _c["owed_val_A"] + _c["owed_val_B"], "err": _c["err_val_A"] + _c["err_val_B"],
                                                              "owed_n": _c["owed_A"] + _c["owed_B"], "err_n": _c["err_A"] + _c["err_B"]}
BANDS_USD = ((25_000, "#dfe6ec", "<$25K"), (100_000, "#b9cad8", "$25–100K"), (500_000, "#d6a29e", "$100–500K"), (2_000_000, "#b25a55", "$500K–2M"))
def _shade(rate):
    ...
    if r and not r["pending"]:
        ...
        if FLOWS:
            ...
            tip = (f'{r["county"].title()} — leaving the county: ${fl["owed"]:,.0f}/yr across {fl["owed_n"]:,} businesses · '
                   f'arriving in error: ${fl["err"]:,.0f}/yr across {fl["err_n"]:,} · {r["biz"]:,} businesses located') if fl else f'{r["county"].title()} — no cross-county flows'
        else:
...
if FLOWS:
    MAP_LEGEND = ('<div class="maplegend">' + "".join(f'<span><i style="background:{col}"></i>{lab}</span>' for _, col, lab in BANDS_USD) +
                  '<span><i style="background:#8f2f2a"></i>$2M+</span>'
                  '<em>local sales tax appearing to leave the county across county lines each year — Tier A+B businesses at county averages, both halves · hover for detail · click to open the county workbench</em></div>')
